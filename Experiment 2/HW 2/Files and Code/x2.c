...
interrupt [EXT_INT0] void ext_int0_isr(void) {
    second_counter = minute_counter = hour_counter ;
}
...
while (1)
      {
        ...
        if(second_counter == second_read && minute_counter == minute_read && hour_counter == hour_read){
          sprintf(time , " Completed "  );
          second_counter = minute_counter = hour_counter = 0 ;
        }

        else {
          sprintf(time , "%d : %d : %d " , hour_counter,  minute_counter , second_counter);
        }
